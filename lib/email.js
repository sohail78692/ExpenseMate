...
export const sendFeedbackEmail = async (feedbackData) => {
    const transporter = createTransporter();
    const { userName, userEmail, message, type } = feedbackData;

    const subject = type === "issue"
        ? `🚨 Issue Report from ${userName}`
        : `💬 Feedback from ${userName}`;

    const mailOptions = {
        from: `"ExpenseMate" <${process.env.EMAIL_USER}>`,
        to: "sohail786akh@gmail.com",
        replyTo: userEmail,
        subject: subject,
        html: `
            <!DOCTYPE html>
            ...
            <head>
                ...
                <style>
                    ...
                    .header { background-color: #f4f4f4; padding: 10px; border-radius: 8px 8px 0 0; border-bottom: 1px solid #ddd; }
            ...
            <body>
                <div class="container">
                    <div class="header">
                        <h2>${type === "issue" ? "🚨 Issue Report" : "💬 New Feedback"}</h2>
                    </div>
                    <div class="content">
                        <p><span class="label">From:</span> ${userName} (${userEmail})</p>
                        <hr/>
                        <p><span class="label">Message:</span></p>
                        <p style="white-space: pre-wrap; background-color: #f9f9f9; padding: 10px; border-radius: 4px;">${message}</p>
                    </div>
                    <div class="footer">
                        <p>Sent from ExpenseMate Profile Section</p>
                    </div>
                </div>
            </body>
            </html>
        `,
        text: `
${type === "issue" ? "ISSUE REPORT" : "FEEDBACK"}
------------------------------------------------
